function Simulator(data::Matrix, privatepeoplesub::Matrix, privatedemandmodelparameters::Array{Float64, 2}, medicaidpeoplesub::Matrix, medicaiddemandmodelparameters::Array{Float64, 2}, year::Int64, mkt_fips::Int64; T = 100, sim_start = 2, fields = 8, neighbors_start = 108, entrants = [0, 1, 2, 3], entryprobs = [0.9895, 0.008, 0.0005, 0.002], fipscodeloc = 78, yearloc = 75, level1_hospitals0loc = 11, fidloc = 74, level2solo_hospitals0loc = 10, level3_hospitals0loc = 9, act_intloc = 79, act_sololoc = 80, lev105loc = 97, lev205loc = 98, lev305loc = 99, lev1515loc = 101, lev2515loc = 102, lev3515loc = 103, lev11525loc = 105, lev21525loc = 106, lev31525loc = 107, v15loc = 94, v16loc = 95, idloc = 1, facilityloc = 82, locationloc = 88, firstyearloc = 91, cityloc = 85,  TotalBeds2loc = 20, TotalBeds1loc = 4)
  ...
  #            println("Bad Action Chosen by", el)
            ...
            #state_history[i, (fid-1)*fields + 6] = privatedemand # handled below, not here.
            #state_history[i, (fid-1)*fields + 7] = medicaiddemand # handled below, not here.
            state_history[i, (fid-1)*fields+8] = 0 #perturbation
  ...
  #            println("Value Exception at ", level1, " ", level2, " ", level3, " ", mkt_fips, " year", year)
            ...
            #state_history[i, (fid-1)*fields + 6] = privatedemand # Not recorded here - recorded below.
            #state_history[i, (fid-1)*fields + 7] = medicaiddemand # Not recorded here - recorded below.
            state_history[i, (fid-1)*fields+8] = 0 #perturbation
  ...
  #          print("Exit recorded \n")
            ...
            #state_history[i, (fid-1)*fields + 7] = demand # no demand realized - exited. Not recorded here - recorded below.
            state_history[i, (fid-1)*fields+8] = 0 #perturbation
            # Set own distance counts to 0 for all categories
            (data[a,lev105loc], data[a,lev205loc], data[a,lev305loc], data[a,lev1515loc], data[a,lev2515loc], data[a,lev3515loc], data[a,lev11525loc], data[a,lev21525loc], data[a,lev31525loc]) = zeros(1,9)
          end
