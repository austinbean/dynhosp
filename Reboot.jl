...
# println("Testing Demand - test for deterministic component, demtest for random component")
# test = DetUtil(pinsured, privatedemandmodelparameters)
# demtest = DemandModel(test, privatedemandmodelparameters, Array{Float64, 2}())
# println("Check Demand Counter - demcount ")
# demcount = DemandCounter(demtest)
# println(sum(demcount[2:end, :]))
# println("Testing WTP Computation - testWTP")
# testWTP = ComputeWTP(test)
# println("WTP Mapping - mappedWTP")
# mappedWTP = MapWTP(testWTP)
# println("Two tests for MapWTP - maptest1, maptest2")
# maptest1 = MapCheck(mappedWTP)
# maptest2 = MapIndividualCheck(mappedWTP)
# println("Check Return WTP - sumWTP")
# sumWTP = ReturnWTP(mappedWTP)
# println("Testing Simulator - a1")
...
# println("Testing PerturbSimulator  - b1")
...
# println("Testing logitest - l1")
# l1 = logitest( (1, 0), 2, 3, 1, zeros(9))
# println("Testing DemandModel with Entrants - deme1")
# deme1 = DemandModel(test, privatedemandmodelparameters, [99999 1 0 120 32.96  -96.8385], true) #implicitly tests EntrantsU as well
# println("Testing DynamicValue - val1")
