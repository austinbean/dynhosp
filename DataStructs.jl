...
#TODO: rewrite this - take the whole state as an argument.  Return a big dictionary for the whole state, not each zip code.
...
function GenPChoices(zipc::zip; dist_μ = 0, dist_σ = 1, dist_ξ = 0, d = Distributions.GeneralizedExtremeValue(dist_μ, dist_σ, dist_ξ))
  ...
  #TODO: the issue here is that some counties have ZERO facilities.
  if outp.count > 0
...
# TODO: this isn't working as I want it.  It's appending every time to the demand history.
# The problem is in the GenPChoices - this should work on the whole state, not a zipcode.
# It should return a big Dict for the whole state.
...
#      WTPMap(pats, Tex)  #TODO - fix this!
      for zi in keys(pats.zips)
        #TODO: this is dumb.  Pdemandmap works on the whole state.
        #TODO: demandhistory is 190,000 elements long in the 20 period case?  No way.  The blame must be here.
        PDemandMap(GenPChoices(pats.zips[zi]), Tex)
        MDemandMap(GenMChoices(pats.zips[zi]), Tex)
      end
      for elm in el.config
